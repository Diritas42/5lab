...
class Enemy:
    def __init__(self, x, y, patrol_path, walls, debug_mode=False):
        ...
        self.detection_angle = math.radians(DETECTION_ANGLE)
        self.blind_spot_angle = math.pi
        self.direction = 0
    ...
    def report_collision(self, wall, axis):
        ...
        import time
        now = time.time() * 1000
        if now - self.last_collision_report > 1000:
    ...
    def line_intersects_rect(self, x1, y1, x2, y2, rect):
        rx, ry, rw, rh = rect
        return (
            self.line_intersects_line(x1, y1, x2, y2, rx, ry, rx + rw, ry) or
            self.line_intersects_line(x1, y1, x2, y2, rx + rw, ry, rx + rw, ry + rh) or
            self.line_intersects_line(x1, y1, x2, y2, rx, ry + rh, rx + rw, ry + rh) or
            self.line_intersects_line(x1, y1, x2, y2, rx, ry, rx, ry + rh)
        )
    ...
    def render(self, screen, is_alert_mode):
        ...
        # Отрисовка конуса видимости (исправленная)
        if not is_alert_mode:
            center_x = self.x + self.width/2
            center_y = self.y + self.height/2
            
            # Создаем поверхность для конуса видимости
            cone_surface = pygame.Surface((self.detection_range * 2, self.detection_range * 2), pygame.SRCALPHA)
            cone_color = (255, 100, 100, 50) if self.is_vigilance_enhanced else (255, 0, 0, 30)
            
            # Вычисляем точки для треугольника конуса
            start_angle = self.direction - self.detection_angle / 2
            end_angle = self.direction + self.detection_angle / 2
            
            # Точка начала конуса (центр врага)
            start_point = (self.detection_range, self.detection_range)
            
            # Точки конца конуса
            end_point1 = (
                self.detection_range + self.detection_range * math.cos(start_angle),
                self.detection_range + self.detection_range * math.sin(start_angle)
            )
            end_point2 = (
                self.detection_range + self.detection_range * math.cos(end_angle),
                self.detection_range + self.detection_range * math.sin(end_angle)
            )
            
            # Рисуем треугольник конуса
            pygame.draw.polygon(cone_surface, cone_color, [start_point, end_point1, end_point2])
            
            # Накладываем поверхность на экран
            screen.blit(cone_surface, (center_x - self.detection_range, center_y - self.detection_range))
